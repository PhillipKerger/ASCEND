...
from ascend_math_agent.config import load_config
from ascend_math_agent.initialization import InitializationError, initialize_project


def test_init_creates_complete_local_scaffold_without_secrets(tmp_path: Path) -> None:
    result = initialize_project(tmp_path)
    config = (tmp_path / "ascend.toml").read_text(encoding="utf-8")
    ...
    resolved = load_config(tmp_path / "ascend.toml", env={})
...
def test_init_refuses_to_overwrite_config_without_force(tmp_path: Path) -> None:
    (tmp_path / "ascend.toml").write_text("sentinel = true\n", encoding="utf-8")
    with pytest.raises(InitializationError, match="--force"):
        initialize_project(tmp_path)
    assert (tmp_path / "ascend.toml").read_text() == "sentinel = true\n"
